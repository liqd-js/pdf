...
const borderDisassemble = ( prop, value ) => 
{
...
const spaceBetweenDisassemble = ( prop, value ) => 
{
...
const DISASSEMBLE_STYLES = ( prop, value ) => 
{
    switch (prop) {
        ...
        case 'borderBottom':
        case 'borderRight': 
            return borderDisassemble( prop, value ); break;
        case 'border': 
            return [ 'borderTop', 'borderLeft', 'borderBottom', 'borderRight' ].reduce( ( prev, curr ) => 
            {
                return { ...prev, ...borderDisassemble( curr, value ) }
            }, {} ); break;
        case 'padding':
        case 'margin': 
            return spaceBetweenDisassemble( prop, value ); break;
        default:
            break;
    }
...
function parseDefinition( definition )
{
    ...
    {
        const disassembleStyles = DISASSEMBLE_STYLES( normalizeProperty( property.name ), property.value );
        if( disassembleStyles )
        {
            Object.assign( definition, disassembleStyles )
        }
        else
        {
