...
function compress_inline( inline )
{
...
class Document
{
    #pdf; #document; #virtual_document; #style;
    ...
    constructor( pdf )
    {
        ...
        this.#document = new PDFKit({ size: 'A4', bufferPages: true, autoFirstPage: false, margin: 30 });
        this.#virtual_document = new PDFKit({ size: 'A4', bufferPages: true });
    ...
    #addPage()
    {
        ...
        let header = this.#pdf.header && this.#pdf.header({ $page });
        let footer = this.#pdf.footer && this.#pdf.footer({ $page });
...
module.exports = class PDF
{
    ...
    render( data, options = {})
    {
        ...
        let header = this.#header ? ( props = {}) => PDFParser.parse( this.#template.render( this.#header, { ...options, props: { ...( options.props || {}), data: { ...data, ...props }}})) : null;
        let footer = this.#footer ? ( props = {}) => PDFParser.parse( this.#template.render( this.#footer, { ...options, props: { ...( options.props || {}), data: { ...data, ...props }}})) : null;
