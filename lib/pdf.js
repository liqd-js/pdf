...
function compress_inline( inline )
{
    ...
    for( let i = 0; i < inline.length - 2; ++i )
    {
...
class Document
{
    ...
    constructor( pdf )
    {
        ...
        this.#document = new PDFKit({ size: 'A4', bufferPages: true });
        this.#virtual_document = new PDFKit({ size: 'A4', bufferPages: true });

        let style = new Style( 'font-size: 10px; text-align: left; color: black; font-family: Helvetica;');

        this.#block({ nodes: this.#compile( pdf.main.nodes, style ), style }, 400 );
    ...
    {
        this.#document.font( node.style.fontFamily + ( node.style.fontWeight === 'bold' ? '-Bold' : '' ))
