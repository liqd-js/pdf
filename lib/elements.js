...
const Image = module.exports.Block = class Image extends Element
{
    #src
...
const Block = module.exports.Block = class Block extends Element
{
    ...
    constructor( document, style, width, height, elements )
    {
        super( document, style, width, height );
        ...
        this.#elements = elements.map( n => 
        {
            if( n.type === 'block' )
            {
                ...
                return new Block( document, n.style, n.style.width && n.style.width.endsWith('%') ? this.innerWidth * parseFloat( n.style.width ) / 100 : this.innerWidth, undefined, n.elements );
            }
            else if( n.type === 'grid' )
            {
                return new Grid( document, n.style, n.style.width && n.style.width.endsWith('%') ? this.innerWidth * parseFloat( n.style.width ) / 100 : this.innerWidth, undefined, n.elements );
            }
            else if( n.type === 'text' )
            {
        ...
        for( let row of this.#grid )
        {
            for( let i = 0; i < row.cells.length; ++i )
            {
                if( row.cells[i]?.block )
                {
                    ...
                    row.cells[i].block = ( block = new Block( this.document, block.style, width, undefined, block.elements ));
