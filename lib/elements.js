...
const Text = module.exports.Text = class Text extends Element
{
    ...
    constructor( document, style, width, height, strings )
    {
        ...
        console.log( 'string', require('util').inspect( strings, { depth: Infinity, colors: true }));
...
const List = module.exports.List = class List extends Element
{
    #elements; #type; #attributes; 
    #space = 0; #gap = 0; 
    #fonts = []; #startIndex = 0;
    constructor( document, style, width, height, element, options )
    {
        super( document, style, width, height );
        ...
        if( this.#type === 'ol' )
        {
            this.#startIndex = parseInt( this.#attributes?.start || '0' );
        }
    ...
    render( x, y )
    {
        ...
        for( const [ index, element ] of this.#elements.entries() )
        {
            ...
            else
            {
                ...
                const label = this.#startIndex + index;
                const strings = [ { text: `${ label + ( label >= 0 ? 1 : 0 ) }. `, style: style } ]
